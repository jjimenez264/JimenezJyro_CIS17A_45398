...
Stats *stat(const Array *, int);     //Find & store mean, median, & modes in structure
...
int main(int argc, char*argv[]) {
    ...
    Stats *stats=stat(array, arySize);
...
Stats *stat(const Array *array, int arySize){
    ...
    stats->mode->data=new int;
    //Stub for no modes
    stats->mode->size = 0;
    //Initialize frequency and number of modes
    int freq, max, nModes;
    max = 1;
    nModes = 0;
    //Find Max Frequency
    ...
    //Divide all data by size of array
    stats->avg = stats->avg / arySize;
